...
void shim::add_stdlib_shimmed_symbols(std::vector<shim::shimmed_symbol> &list) {
    list.insert(list.end(), {
        ...
        // div/ldiv/lldiv and abs/labs/llabs were never added here either.
        // These are safe to forward directly: the div_t/ldiv_t/lldiv_t structs
        // are just {quotient, remainder} pairs of matching primitive types on
        // both bionic and glibc, so there's no ABI layout risk like the
        // sigset_t-based signal functions have.
        {"div", div},
        {"ldiv", ldiv},
        {"lldiv", lldiv},
        {"abs", abs},
...
void shim::add_signal_shimmed_symbols(std::vector<shim::shimmed_symbol> &list) {
    list.insert(list.end(), {
        ...
        // pthread_sigmask was never added alongside sigprocmask above, so any NDK
        // library that imports it (e.g. current libminecraftpe.so builds) fails to
        // load with "cannot locate symbol pthread_sigmask" on hosts where it's not
        // separately resolvable. Stub it the same way sigprocmask is stubbed above
        // (see libc-shim#9 for why these are no-ops instead of real forwarding calls).
        {"pthread_sigmask", +[](int __how, const void * __set, void * __oset) -> int {
            return 0;
        }},
