...
void JniSupport::initPairip() {
    auto pc = linker::dlopen("libpairipcore.so", 0);
    if(!pc) {
        Log::info("JniSupport", "No libpairipcore.so present, skipping PairIP init");
        return;
    }
    typedef jint (*jni_onload_fn)(JavaVM *, void *);
    auto onload = (jni_onload_fn)linker::dlsym(pc, "JNI_OnLoad");
    if(!onload) {
        Log::info("JniSupport", "libpairipcore.so has no JNI_OnLoad, skipping PairIP init");
        return;
    }
    FakeJni::LocalFrame frame(vm);
    Log::info("JniSupport", "Calling libpairipcore.so JNI_OnLoad for PairIP init");
    jint rc = onload((JavaVM *)&vm, nullptr);
    Log::info("JniSupport", "libpairipcore.so JNI_OnLoad returned %d", rc);
}
