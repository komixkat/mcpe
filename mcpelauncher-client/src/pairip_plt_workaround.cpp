...
// The resolver is only reachable from the naked trampoline's inline asm below,
// which the compiler cannot see. Under LTO (-flto) the definition would be
// dropped as unused and the link fails with "undefined reference"; 'used'
// forces emission. 'noinline' keeps the call out of any optimizer path.
...
void apply_pairip_plt_workaround() {
    ...
    apply_pairip_plt_workaround_for(pfm_base, "libPlayFabMultiplayer.so");
}

void apply_pairip_plt_workaround_for(uintptr_t base, const char* libname) {
    // The linker's pre-constructor hook may fire before the fully wrapper
    // path (which resolves the base) once the library has been linked. Keep
    // this idempotent across both entry points.
    static bool workaround_applied = false;
    if (workaround_applied) {
        return;
    }

    const uintptr_t pfm_base = base;
    const char* realpath = libname;
    ...
    // Do NOT restore .got.plt to read-only: the lazy PLT resolver
    // (mcpelauncher_pairip_plt_resolve_handler) still needs to write resolved
    // addresses / no-op stubs into these slots at runtime. Re-locking the
    // page would make those stores fault with SIGSEGV.
...
void apply_pairip_plt_workaround_for(uintptr_t base, const char* libname) {
    // No-op on non-x86_64 architectures (see apply_pairip_plt_workaround).
}
